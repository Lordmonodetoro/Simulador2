# -*- coding: utf-8 -*-
"""codigo app rev3_iterativo.ipynb
...
class PlantaAzucareraCompleta:
    ...
    def mod_6_7_cocimiento_y_refundicion(self, m1, m3, m5):
        ...
        # Purezas dinámicas desde configuración
        P_TJ = 91.60; P_JF = 91.60
        P_A = 99.80
        P_B = float(c.get('IN_Pureza_AzucarB_pct', 98.70))
        P_C = float(c.get('IN_Pureza_AzucarC_pct', 96.70))
        P_Mol = float(c.get('IN_Pureza_Melaza_pct', 57.20))
        P_GA = 83.70; P_GB = 76.20
    ...
    def mod_5_evaporacion(self, m4, m3, m6, m7, m1, m2):
        ...
        cp_jugo = 0.94
        cp_agua = 1.00
        SANGRIA = float(c.get('OP_Venteos_Sangria_th', 0.30)) # Venteo dinámico
        ...
        def simular_cascada(E0):
            ...
            extr_base_caldera = float(c.get('OP_Retorno_Condensado_Caldera_th', 5.08))
            ...
            for i in range(1, 6):
                ...
                extr = extr_base_caldera * (W / 369.46) if i == 1 else 0.0
    ...
    def mod_8_condensados_agua(self, m5, m6, m4, m3, m1, m2, m7):
        ...
        # Scrubber parametrizado dinámicamente
        scrubber_base = float(c.get('OP_Scrubber_Flujo_th', 11.50))
        scrubber_th = float(m1.get('OUT_CanaProcesada_th', 445.0)) * (scrubber_base / 445.0)
    ...
    # ====================================================================
    # SOLVER ITERATIVO DINÁMICO (Sustituto del for loop de 5 iteraciones)
    # ====================================================================
    def simular(self, tol=1e-4, max_iter=100):
        # 1. Resolución de módulos lineales o secuenciales puros
        m1 = self.mod_1_difusiones()
        m2 = self.mod_2_calentamiento_crudo(m1)
        
        # Inicialización de diccionarios de estado
        ...
        for iteracion in range(max_iter):
            ...
            m6, m7 = self.mod_6_7_cocimiento_y_refundicion(m1, m3, m5_prev)
            m5 = self.mod_5_evaporacion(m4, m3, m6, m7, m1, m2)
            m8 = self.mod_8_condensados_agua(m5, m6, m4, m3, m1, m2, m7)
            
            # Evaluación dinámica del error
            demanda_vapores = m5.get('OUT_Balance_Vapores_Demanda', {})
            vapor_total_actual = sum(demanda_vapores.values()) if demanda_vapores else 0.0
            
            if vapor_total_actual > 0:
                error_relativo = abs((vapor_total_actual - vapor_total_prev) / vapor_total_actual)
            else:
                error_relativo = 1.0 
            ...
            vapor_total_prev = vapor_total_actual

        if not convergencia_alcanzada:
            print(f"ADVERTENCIA: El modelo no convergió tras {max_iter} iteraciones. Error: {error_relativo:.4f}")

        # 3. Módulos finales dependientes del estado convergido
        m9 = self.mod_9_energia(m1, m4, m5, m6)

        # 4. Casteo riguroso de Python Floats
...
# Valores actualizados de acuerdo con ACOR_Balance.pdf
config_usuario = {
    'IN_Molienda_th': 445.0, 'IN_Riqueza_Remolacha_pct': 17.4, 'IN_Pureza_Agricola_pct': 90.4,
    'IN_Marc_Fibra_pct': 3.55, 'OP_DifPren_Ratio_Extraccion': 1.11, 'OP_DifPren_MS_PulpaPrensada_pct': 27.5,
    ...
    'OP_Evaporacion_BrixSalida_objetivo_pct': 69.4, 'OP_Venteos_Sangria_th': 0.30, 'OP_Retorno_Condensado_Caldera_th': 5.08,
    'OP_Cocimiento_BrixMasaA_pct': 91.0, 'OP_Cocimiento_BrixMasaB_pct': 94.6, 'OP_Cocimiento_BrixMasaC_pct': 95.3,
    'IN_Pureza_AzucarB_pct': 98.70, 'IN_Pureza_AzucarC_pct': 96.70, 'IN_Pureza_Melaza_pct': 57.20,
    'OP_Calentador15_Vapor_Fuente': 'Vapor_4toEfecto', 'OP_Scrubber_Flujo_th': 11.50,
...
rendimiento = (azucar_producida / config_usuario['IN_Molienda_th']) * 100
...
with st.sidebar.expander("🌱 Materia Prima & Módulo 1 (Difusión)", expanded=False):
    ...
    in_marc = st.slider("Marc_Fibra_pct (%)", 3.0, 7.0, 3.55, 0.01)
...
with st.sidebar.expander("🧪 Módulo 3 y 8 (Depuración y Configuración de Agua)", expanded=False):
    ...
    op_scrubber = st.number_input("Flujo Scrubber Horno Cal (t/h)", value=11.50)
...
with st.sidebar.expander("💨 Módulo 5 (Evaporación)", expanded=False):
    ...
    op_sangria = st.number_input("Venteos / Sangría por efecto (t/h)", value=0.30)
    op_retorno_caldera = st.number_input("Retorno Condensado Caldera V1 (t/h)", value=5.08)
...
with st.sidebar.expander("🍬 Módulo 6 (Cocimiento) & 9 (Energía)", expanded=False):
    ...
    in_pur_b = st.number_input("Pureza Azúcar B (%)", value=98.70)
    in_pur_c = st.number_input("Pureza Azúcar C (%)", value=96.70)
    in_pur_mol = st.number_input("Pureza Melaza (%)", value=57.20)
...
config_usuario.update({
    ...
    'OP_Evaporacion_BrixSalida_objetivo_pct': op_evap_brix_obj, 'OP_Venteos_Sangria_th': op_sangria, 'OP_Retorno_Condensado_Caldera_th': op_retorno_caldera,
    ...
    'IN_Pureza_AzucarB_pct': in_pur_b, 'IN_Pureza_AzucarC_pct': in_pur_c, 'IN_Pureza_Melaza_pct': in_pur_mol,
