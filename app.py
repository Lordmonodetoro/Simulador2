# -*- coding: utf-8 -*-
"""motor_termodinamico.py

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1R6K0M6ygPecrsjgaDpe_QNepm5qssBnM
"""
...
# ====================================================================
# DISEÑO DE LA APLICACIÓN WEB EN STREAMLIT CON HERRAMIENTAS VISUALES
# ====================================================================
st.title("🏭 Gemelo Digital Planta ACOR 2026")
st.markdown("Ajusta los **parámetros de entrada** en la barra lateral y observa en tiempo real los resultados del balance termodinámico, métricas ejecutivas y flujos visuales.")
...
v_sobre_rem = resultados['M9'].get('OUT_KPI_VaporSobreRemolacha_pct', 0.0)
...
kpi1.metric(label="⚡ Vapor / Remolacha", value=f"{v_sobre_rem:.2f}%", delta="Nominal ~38%")
kpi2.metric(label="🍬 Rendimiento Azúcar", value=f"{rend_az:.2f}%", delta="Objetivo >15%")
kpi3.metric(label="🔋 Potencia Eléctrica", value=f"{pot_mw:.2f} MW", delta="Cogeneración")
kpi4.metric(label="📦 Producción Azúcar A", value=f"{az_comercial:.2f} t/h", delta="Comercial")

st.markdown("---")

# ====================================================================
# DIAGRAMA DE SANKEY INTERACTIVO (FLUJOS DE VAPOR Y JUGO)
# ====================================================================
st.markdown("### 🌊 Diagrama de Flujo Térmico y de Masas (Sankey)")

f_remolacha = in_molienda
f_jugo_crudo = resultados['M1'].get('OUT_Corriente_JugoCrudo_Flujo_th', 0)
f_jugo_fino = resultados['M3'].get('OUT_Corriente_JugoFinoTotal_Flujo_th', 0)
f_jarabe = resultados['M5'].get('OUT_Corriente_Jarabe_Flujo_th', 0)
v_calderas = resultados['M9'].get('OUT_Caldera_VaporVivoTotal_th', 0)
v_evap_tot = resultados['M5'].get('OUT_Evaporacion_AguaTotalEvaporada_th', 0)
...
fig_sankey = go.Figure(go.Sankey(
    node = {
      ...
      "label": ["Remolacha (M1)", "Vapor Calderas", "Jugo Crudo (M1-M2)", "Jugo Fino (M3-M4)", "Estación Evaporación (M5)", "Agua Evaporada", "Jarabe / Melga (M5-M7)"],
      "color": ["#2ca02c", "#d62728", "#ff7f0e", "#1f77b4", "#9467bd", "#17becf", "#8c564b"]
    },
    link = {
      "source": [0, 1, 2, 3, 4, 4],
      "target": [2, 4, 3, 4, 5, 6],
      "value": [f_remolacha, v_calderas, f_jugo_crudo, f_jugo_fino, v_evap_tot, f_jarabe]
    }
))

fig_sankey.update_layout(title_text="Esquema General de Masas y Vapores principales (t/h)", font_size=12, height=400)
